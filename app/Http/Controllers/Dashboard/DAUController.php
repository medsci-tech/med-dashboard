...
class DAUController extends Controller
{
    ...
    public function handleRequest(DAURequest $request)
    {
        ...
        foreach ($dates as $date) {
            $countArray[$date] = sizeof(MimeLoginLog::where('action', 'login')->whereBetween('created_at', [date_create($date), date_create(date("Y-m-d", strtotime("+1 day", strtotime($date))))])->distinct('phone')->get());
        }
